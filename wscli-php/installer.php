...

{
    ...
    if ($item->sha1 !== hash("sha1", file_get_contents($item->name))) {
        unlink($item->name);
    ...
    /*
    echo " - Copying wscli.phar executable to /usr/local/bin/ ...$n";

    try {
        copy($item->name, "/usr/local/bin/wscli");
    } catch (Exception $e) {
        echo " x Installing the binary failed, check permissions.$n$n";
